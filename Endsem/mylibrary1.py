...
def ODE_Predictor_corrector(f, a, b, h, x0, y0):
    y = [y0]
    x_n = x0
    n = int((b - a) / h)
    for i in range(n):
...
def forward_Euler(f, a, b, h, x0, y0):
    y = [y0]
    x_n = x0
    n = int((b - a) / h)
    for i in range(n):
...
def PDE_HeatEqn_Solve(g,t_0,t_f,x_0,x_l,h_x,h_t):
    a = h_t/(h_x**2)
    #Define the grid
    X = []
    T = []
    ...
    V_mat.append(u0)
    #Define A
    A = [[0.0 for i in range(N+1)] for j in range(N+1)]
    for i in range(N+1):
        for j in range(N+1):
            if i == j:
                A[i][j] += 1-2*a
    ...
    #Find V_j and append to V_mat
    for i in range(T_n):
        V = matrix_multiply(A,V_mat[i])
        V_mat.append(V)
    
    return V_mat, X, T

def Lagrange_interpolation(E_h, E_l, x_h, x_l, x_b):
    E = E_l + (E_h - E_l)*(x_b - x_l)/(x_h - x_l)
    return E
...
def leastsquareslinearfit(X,Y,Sigma = None):
    ...
    Del = S*Sxx - Sx**2
    a1 = float((Sxx*Sy - Sx*Sxy)/Del)
    a2 = float((Sxy*S - Sx*Sy)/Del)

    sigma_a1 = math.sqrt(Sxx/Del)
    sigma_a2 = math.sqrt(S/Del)
...
def LeastsquaresPolynomialFit(X, Y, degree, Sigma = None):
    if Sigma == None:
        Sigma = []
        for i in range(len(Y)):
            Sigma.append(1)
    ...
    coeffs = GJElimination(A,b)  #List of coefficients starting from a0 to an
